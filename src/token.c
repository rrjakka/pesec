...
void token_print(FILE* stream, const token_t token)
{
    switch (token.type)
    {
        ...
        case TOKEN_TYPE_GREATER_EQUALS:
            fprintf(stream, "comparison: %.*s", token.value.as_string.length, token.value.as_string.string); break;
        case TOKEN_TYPE_PLUS:
        case TOKEN_TYPE_MINUS:
        case TOKEN_TYPE_SLASH:
        case TOKEN_TYPE_ASTERISK:
            fprintf(stream, "operator: %.*s", token.value.as_string.length, token.value.as_string.string); break;
        case TOKEN_TYPE_LPAREN:
        case TOKEN_TYPE_RPAREN:
            fprintf(stream, "paren: %.*s", token.value.as_string.length, token.value.as_string.string); break;
        case TOKEN_TYPE_LBRACE:
        case TOKEN_TYPE_RBRACE:
            fprintf(stream, "brace: %.*s", token.value.as_string.length, token.value.as_string.string); break;
